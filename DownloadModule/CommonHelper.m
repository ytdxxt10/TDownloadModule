...
+(NSString *)getTempFolderPathWithBasepath:(NSString *)name
{
    NSString *pathstr = [[self class]getDocumentPath];
    pathstr = [pathstr stringByAppendingPathComponent:name];
    pathstr=[pathstr stringByAppendingPathComponent:[[NSUserDefaults standardUserDefaults]objectForKey:@"USER"]];
...
+(NSString *)getDiskSpaceInfo{
    ...
    if (dictionary)
    {
        ...
        NSString *infostr = [NSString stringWithFormat:@"%.2f GB 可用/总共 %.2f GB", ((totalFreeSpace/1000.0f)/1000.0f)/1000.0f, ((totalSpace/1000.0f)/1000.0f)/1000.0f];
        return infostr;
